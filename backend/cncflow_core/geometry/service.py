...
def _drop_slot_fillet_holes(features):
    slots = [
        ...
        if feat.get("subtype") == "recognized_slot" or feat.get("type") in {"pocket", "slot"}
    ]
    if not slots:
        return features
    kept = []
...
def _hole_axis(feat):
    ...
    if isinstance(axis, dict):
        return (axis.get("x") or 0.0, axis.get("y") or 0.0, axis.get("z") or 0.0)
...
def _quote_extents(features, stock_l, stock_w, stock_h):
    ...
    depths = [
        ...
        for feat in features
        if (feat.get("type") == "hole" or feat.get("subtype") == "recognized_hole")
...
def _is_raw_cylinder_hole(feat):
    ...
    return subtype == "cylindrical_candidate" or fid.startswith("cylinder-")


def _select_quote_holes(features, stock_l=0, stock_w=0, stock_h=0):
    """默认只勾过 is_quote_hole 的孔。cylinder-* / 窗口圆角 / 外圆不勾。"""
    ...
    if not extents[0] or not extents[1]:
        return features
    for feat in features:
        if feat.get("type") != "hole" and feat.get("subtype") != "recognized_hole":
            continue
        ...
        if not is_quote_hole(diameter, depth, hole_type, extents, _hole_axis(feat)):
            feat["selected"] = False
    return features


def apply_quote_default_selection(features, stock_l=0, stock_w=0, stock_h=0):
    """报价默认勾选：肩顶 / 整板 XY / 仅 is_quote_hole。手勾不走这里。"""
