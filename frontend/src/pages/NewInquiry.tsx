...
export function NewInquiry({ go }: { go: (h: string) => void }) {
  ...
  const [fileRow, setFileRow] = useState<{ i: number; kind: "step" | "pdf" } | null>(null)
    ...
    setParts(prev => {
      ...
      for (const f of files) {
        if (isStep(f)) {
          ...
          if (idx >= 0) next[idx] = { ...next[idx], pdf: f }
          else next.push({ ...empty(), pdf: f })
        }
      }
  ...
  async function submit() {
    const usable = parts.filter(p => p.name || p.step)
    if (!usable.length) { setErr("请至少添加一个零件或上传 STEP"); return }
    setErr(""); setBusy(true)
    try {
      const inq = await json<any>("/inquiries", {
        method: "POST",
        body: JSON.stringify({ customer, project, due_date: due, title: project || rfq || "新建询价" }),
      })
      for (const p of usable) {
        await json(`/inquiries/${inq.id}/parts`, {
          method: "POST",
          body: JSON.stringify({
            name: p.name || p.step?.name || "零件",
            qty: Number(p.qty) || 1,
            material: p.material,
            blank_type: "板料",
  ...
  return <div className="space-y-6">
    <button className="text-sm text-blue-600" onClick={() => go("")}>← 返回报价工作台</button>
    <div>
      <div className="font-mono text-[10px] uppercase tracking-[.18em] text-slate-500">CREATE RFQ / STEP 01</div>
      <h1 className="mt-2 text-3xl font-semibold text-slate-800">新建报价</h1>
    ...
    <Card className="p-5">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <label className="text-xs text-slate-500">客户<Input className="mt-1" value={customer} onChange={e => setCustomer(e.target.value)} placeholder="华南精密制造" /></label>
    ...
    <Card className="p-5">
      <div className="mb-4 flex items-center justify-between">
        ...
        <button className="text-sm text-blue-600" onClick={() => setParts(p => [...p, empty()])}>+ 添加零件</button>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          <thead><tr className="border-b border-[#e2e8f0] text-xs text-slate-500">
            <th className="py-2 pr-2">零件名称</th><th>数量</th><th>材料</th><th>表面处理</th><th>关键精度</th><th>粗糙度</th><th>图纸</th><th>状态</th><th></th>
          </tr></thead>
          <tbody>
            {parts.map((p, i) => (
              <tr key={i} className="border-b border-slate-100">
                <td className="py-2 pr-2"><Input value={p.name} onChange={e => upd(i, { name: e.target.value })} placeholder="Housing" /></td>
                <td className="w-20"><Input type="number" value={p.qty} onChange={e => upd(i, { qty: e.target.value })} /></td>
                <td><Select value={p.material} onChange={e => upd(i, { material: e.target.value })}>
                  <option>AL6061-T6</option><option>AL7075</option><option>SUS304</option><option>POM</option><option>铝合金</option><option>钢</option><option>不锈钢</option>
                </Select></td>
                ...
                <td className="whitespace-nowrap">
                  <button className="mr-2 text-xs text-blue-600" onClick={() => { setFileRow({ i, kind: "step" }); stepRef.current?.click() }}>{p.step ? p.step.name : "STEP"}</button>
                  <button className="text-xs text-blue-600" onClick={() => { setFileRow({ i, kind: "pdf" }); pdfRef.current?.click() }}>{p.pdf ? p.pdf.name : "PDF"}</button>
                </td>
                <td className="text-xs text-slate-500">待分析</td>
                <td><button className="text-xs text-slate-500 hover:text-red-600" onClick={() => setParts(ps => ps.filter((_, idx) => idx !== i))}>删除</button></td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
      <input ref={stepRef} type="file" accept=".step,.stp" className="hidden" onChange={e => { const f = e.target.files?.[0]; if (f && fileRow) upd(fileRow.i, { step: f, name: parts[fileRow.i].name || stem(f) }); e.target.value = "" }} />
      <input ref={pdfRef} type="file" accept=".pdf" className="hidden" onChange={e => { const f = e.target.files?.[0]; if (f && fileRow) upd(fileRow.i, { pdf: f }); e.target.value = "" }} />
      <div
        onDragEnter={e => { e.preventDefault(); setDrag(true) }}
        onDragOver={e => e.preventDefault()}
        onDragLeave={() => setDrag(false)}
        onDrop={e => { e.preventDefault(); setDrag(false); addFiles(e.dataTransfer.files) }}
        className={`mt-4 grid min-h-[140px] place-items-center rounded border border-dashed p-6 text-center ${drag ? "border-blue-600 bg-blue-50" : "border-[#e2e8f0] bg-[#f8fafc]"}`}
      >
        <div>
          <UploadCloud className="mx-auto mb-2 text-slate-400" size={28} />
          <div className="text-sm text-slate-700">拖拽 STEP / PDF 到这里，或点击添加零件后上传图纸</div>
    ...
    <div className="flex justify-end gap-3">
      <Button variant="outline" onClick={() => go("")}>取消</Button>
      <Button onClick={submit} disabled={busy}>{busy ? "提交中" : "开始 AI 分析"}</Button>
    </div>
